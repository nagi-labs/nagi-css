...
**進行中**: #10（値のトークン化）は議論のうえ段階実装に着手。第1段（境界検査）は実装済み。
...
| **一部実装**（このブランチ） | #10 — 第1段の `unknown-token` / `token-layer`（トークンは出荷せず、境界だけを検査）。第2段以降（`value-token-required` / `local-value-declaration`）は未着手 |
| **未着手** | #10 の第2段以降のみ |
...
| 10 | 値（トークン）の canonical form が未検査 | 未実装 | P2 | **一部実装**（第1段のみ） |
...
### 10. 値（トークン）の canonical form が未検査 — **一部実装**（第1段）
...
**未着手（第2段以降、この順で）**:
- `value-token-required` — 色から。色に `--local-*` の逃げ道は与えない（一度きりの色は無い）
- `local-value-declaration` — 長さの `--local-*` 経由の許可（光学補正など）
- 絶対長への拡大 — `max-inline-size: 32rem` のような一度きりの寸法があるため、ここは論点が残る

**切り分けの規則（第2段の設計、据え置き）**: CSS の値空間が既に有限なプロパティは検査不要
（`display` `position` `overflow` …）、無限なプロパティはトークン必須。無限側も機械判定できる —
絶対量（px/rem/em/ch…）と色は**デザイン決定**でトークン必須、相対比（`%` `fr` 無単位 `16 / 9`）と
キーワードは**構造決定**で素のまま可、原始定数（`0` `auto` `none` `100%`）は allowlist。

**未決定**: `var(--color-text, #333)` のフォールバックを error にするか warning にするか（第2段でのみ影響）。
...
**#10（値のトークン化）以外はすべて着手済み**。残る議論は #10 のみ。

派生として未着手のもの:

- `<b>` `<i>` `<u>` `<s>` を `bannedClasses` に寄せる（#23 の派生）— 既存コードを弾く変更なので分離
- #20 の完全版：読めない `:class` 式（`toneClass`、`` `-${tone}` ``）そのものを拒否する。
  README の適合例 `:class="iconName"` を壊すため、設定でアイコン接頭辞を宣言する前提が必要
