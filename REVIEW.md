...
**進行中**: #10（値のトークン化）は議論のうえ段階実装に着手。第1段（境界検査）と第2段（色）は実装済み。
...
| **一部実装**（このブランチ） | #10 — 第1段 `unknown-token` / `token-layer`（トークンは出荷せず、境界だけを検査）、第2段 `value-token-required`（色）。第3段（長さの `--local-*` 逃げ道と絶対長）は未着手 |
| **未着手** | #10 の第3段（長さ）のみ |
...
| 10 | 値（トークン）の canonical form が未検査 | 未実装 | P2 | **一部実装**（第1・2段） |
...
### 10. 値（トークン）の canonical form が未検査 — **一部実装**（第1・2段）
...
**未着手（第3段）**: 長さ。`local-value-declaration`（`--local-*` 経由の許可、光学補正など）と
絶対長への拡大。`max-inline-size: 32rem` のような一度きりの寸法があるため、ここは論点が残る。

**切り分けの規則（第3段の設計、据え置き）**: CSS の値空間が既に有限なプロパティは検査不要
